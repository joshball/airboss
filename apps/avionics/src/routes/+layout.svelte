...
$effect(() => {
	...
	try {
		authBannerDismissed = window.sessionStorage.getItem(AVIONICS_STORAGE_KEYS.AUTH_BANNER_DISMISSED) === '1';
	} catch {
...
function dismissAuthBanner() {
	...
	if (typeof window === 'undefined') return;
	try {
		window.sessionStorage.setItem(AVIONICS_STORAGE_KEYS.AUTH_BANNER_DISMISSED, '1');
	} catch {
...
// `resolveThemeSelection` enforces route safety locks. Avionics does
// not lock the theme on any route -- the picker stays fully functional
// in light and dark on every page.
...
// follows the user's pick without waiting for a reload.
$effect(() => {
