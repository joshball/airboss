...
/**
 * Sim's better-auth instance. Sim does not host the login UI -- that
 * lives on study (`study.airboss.test`). Sim only needs to *read* the
 * cross-subdomain `bauth_session_token` cookie and resolve it back to
 * a user, so it can attribute persisted sim attempts to the right
 * learner.
 *
