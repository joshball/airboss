...
<section class="page">
	...
	<PageExplainer pageKey={PAGE_EXPLAINER_KEYS.PROGRAM_GOAL}>
		...
		is a slice of study you're focused on right now ("Pass PPL written by July"). Each goal pins one or more
		<Tooltip for="qual">quals</Tooltip>
		and one or more
		<Tooltip for="syllabus">syllabi</Tooltip>; the active goal anchors the daily session.
	</PageExplainer>
	...
	{#if totalGoals === 0}
		...
		>
			{#snippet actions()}
				<Button href={ROUTES.PROGRAM_GOALS_NEW} variant="primary">Create your first goal</Button>
			{/snippet}
		</EmptyState>
	{:else}
				...
				<section class="bucket" aria-labelledby="bucket-{bucketKey}">
					...
					<ul class="goal-list">
						{#each goals as goal (goal.id)}
							<li class="goal">
								<a class="goal-link" href={ROUTES.PROGRAM_GOAL(goal.id)}>
