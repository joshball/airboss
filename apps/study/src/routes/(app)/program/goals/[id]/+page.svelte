...
// `data.activePlan` is contributed by `/program/+layout.server.ts` and merged
// into `PageData` by SvelteKit's generated types -- no explicit intersection
// needed. See `docs/work/reviews/2026-05-05-study-ia-cleanup-phase2-svelte.md`
// S-1 for the rationale.
...
<section class="page">
	...
	>
		{#snippet actions()}
			...
			{:else}
				...
				<span data-testid="goal-detail-start-cta">
					{#if data.activePlan}
						<Button href={ROUTES.SESSION_START} variant="primary">Start studying</Button>
					{:else}
						<Button href={ROUTES.PROGRAM_PLANS_NEW} variant="primary">Build my plan</Button>
					{/if}
				</span>
			{/if}
		{/snippet}
	...
	{#if !editing}
		<PageExplainer pageKey={PAGE_EXPLAINER_KEYS.PROGRAM_GOAL}>
			This is one
			<Tooltip for="goal">goal</Tooltip>. The primary goal anchors your daily
			<Tooltip for="session">session</Tooltip>; secondary goals stay on the bench until you promote them.
		</PageExplainer>
	{/if}
