...
<section class="page">
	<nav class="crumb" aria-label="Breadcrumb">
		<a href={ROUTES.PROGRAM_QUALS}>{NAV_LABELS.PROGRAM_QUALS}</a>
	...
	{#if pinnedSyllabus !== null}
		<aside class="banner pinned" role="status">
			...
			<a class="banner-cta" href={ROUTES.PROGRAM_QUAL(credential.slug)}>Switch to current</a>
		</aside>
	{/if}
			...
			<section class="areas" aria-labelledby="areas-heading">
				...
				<ul class="area-list">
					{#each areas as area (area.areaCode)}
						<li class="area">
							<a class="area-link" href={ROUTES.PROGRAM_QUAL_AREA(credential.slug, area.areaCode)}>
				...
				<div class="prereq-group">
					...
					<ul>
						{#each requiredPrereqs as p (p.credential.id)}
							<li><a href={ROUTES.PROGRAM_QUAL(p.credential.slug)}>{p.credential.title}</a></li>
						{/each}
					</ul>
				</div>
			{/if}
