<script lang="ts">
import { CREDENTIAL_KIND_LABELS, type CredentialKind, PAGE_EXPLAINER_KEYS, ROUTES } from '@ab/constants';
...
<section class="page">
	...
	<PageExplainer pageKey={PAGE_EXPLAINER_KEYS.PROGRAM_QUALS}>
		<Tooltip for="qual">Quals</Tooltip>
		are the certificates and ratings you're working toward (PPL, IR, CPL, CFI). Pick the one that
		describes what you're chasing right now -- that pick anchors your
		<Tooltip for="goal">goal</Tooltip>
		and the
		<Tooltip for="plan">plan</Tooltip>
		that shapes your sessions.
	</PageExplainer>
		...
		<ul class="grid">
			{#each rows as row (row.credential.id)}
				<li class="card" class:primary={row.primaryGoalCredential}>
					<a class="card-link" href={ROUTES.PROGRAM_QUAL(row.credential.slug)}>
