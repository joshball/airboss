...
const goalLink = $derived(data.primaryGoalId ? ROUTES.PROGRAM_GOAL_EDIT(data.primaryGoalId) : ROUTES.PROGRAM_GOALS_NEW);
</script>
...
<section class="page">
	<PageHeader title={plan.title}>
		...
		{/snippet}
		{#snippet actions()}
			<Button variant="ghost" href={ROUTES.PROGRAM_PLANS}>Back to plans</Button>
		{/snippet}
	</PageHeader>
