...
<section class="page">
	...
	>
		{#snippet actions()}
			{#if active}
				<Button variant="secondary" href={ROUTES.PROGRAM_PLAN(active.id)}>Edit plan</Button>
				<Button variant="primary" href={ROUTES.SESSION_START}>Start session</Button>
			{:else}
				<Button variant="primary" href={ROUTES.PROGRAM_PLANS_NEW}>New plan</Button>
			{/if}
		{/snippet}
	</PageHeader>

	<PageExplainer pageKey={PAGE_EXPLAINER_KEYS.PROGRAM_PLAN}>
		A
		<Tooltip for="plan">plan</Tooltip>
		tells the
		<Tooltip for="session">session</Tooltip>
		engine how long, how often, and what to focus on. One plan is active at a time; archived plans stay around as a record
		of what you tried.
	</PageExplainer>
	...
	{#if currentTab === PLAN_STATUSES.ACTIVE}
		{#if active}
			<article class="plan-card active">
				<div class="plan-head">
					...
					<a class="link" href={ROUTES.PROGRAM_PLAN(active.id)}>Open</a>
				</div>
				<dl class="plan-meta">
					<div>
						<dt>Cert goals</dt>
		...
		{:else}
			...
			>
				{#snippet actions()}
					<Button variant="primary" href={ROUTES.PROGRAM_PLANS_NEW}>Create plan</Button>
				{/snippet}
			</EmptyState>
		{/if}
		...
		<article class="card-list">
			...
			<ul class="plans">
				{#each data.archived as p (p.id)}
					<li>
						<a class="plan-name" href={ROUTES.PROGRAM_PLAN(p.id)}>{p.title}</a>
						<span class="meta">
