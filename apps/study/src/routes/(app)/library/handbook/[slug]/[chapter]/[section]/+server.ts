/**
 * `/library/handbook/[slug]/[chapter]/[section]` -- legacy study handbook
 * section reader. 301s to the flightbag section URL with the latest
 * non-superseded edition.
 */
...
export const GET: RequestHandler = async (event) => {
	...
	throw redirect(
		301,
		`${flightbag}${ROUTES.FLIGHTBAG_HANDBOOK_SECTION(documentSlug, edition, chapterCode, sectionCode)}`,
	);
};
