---
id: nav-vor-tracking
title: VOR Tracking
domain: ifr-procedures
cross_domains: [vfr-operations]

knowledge_types: [procedural, perceptual]
technical_depth: working
stability: stable

relevance:
  - cert: PPL
    bloom: apply
    priority: core
  - cert: IR
    bloom: apply
    priority: core
  - cert: CPL
    bloom: apply
    priority: core
  - cert: CFI
    bloom: evaluate
    priority: core

requires: []
deepens: []
applied_by:
  - nav-instrument-approach-structure
  - plan-vfr-cross-country
  - plan-ifr-cross-country
taught_by: []
related:
  - nav-gps-rnav-concepts

modalities: [reading, cards, drill, visualization]
estimated_time_minutes: 60
review_time_minutes: 10

references:
  - source: IPH (FAA-H-8083-16B)
    detail: Instrument Procedures Handbook -- VOR Navigation
    note: Operational use of VORs on airways and approaches.
  - source: AIM
    detail: 1-1-3 -- VHF Omnidirectional Range (VOR)
    note: Service volumes, frequency protection, accuracy limits.
  - source: AFH (FAA-H-8083-3B)
    detail: Chapter 16 -- Navigation Systems
    note: PPL-level VOR orientation, tracking, and interception.

assessable: true
assessment_methods: [demonstration, scenario]
mastery_criteria: >
  Learner can orient to a station from any position, intercept and track a
  radial inbound or outbound with wind correction, recognize a TO / FROM
  indication flip as station passage, and identify a failed or unreliable
  VOR via red flag or VOT check.
---

# VOR Tracking

## Context

<!-- Skeleton. -->

## Problem

<!-- Skeleton. -->

## Discover

<!-- Skeleton. -->

## Reveal

<!-- Skeleton. -->

## Practice

<!-- Skeleton. -->

## Connect

<!-- Skeleton. -->

## Verify

<!-- Skeleton. -->
