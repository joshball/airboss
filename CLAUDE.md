...
- **If a hypothesis-test fix attempt fails twice, STOP iterating from memory.** The Phase 3 crash had five candidate fixes, each from grep + reasoning, each failing to fix the symptom. Anti-pattern #4 from the playbook. After two unsuccessful candidate fixes, switch to measurement: `curl http://127.0.0.1:9600/@fs/<suspect-file>` to read the Vite-served module; walk every value-import in the served output (type-only erases at compile time); identify the actual leak chain there, not in source. If the served chain disagrees with the source-grep, the served chain is authoritative -- Vite sees what the browser sees; grep doesn't.
