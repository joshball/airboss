...
- **Browser-bundled libs must not statically import `node:*` OR reference Node globals like `Buffer` / `process`.** Files in `libs/{constants,utils,types,themes,ui,help,aviation,audit,sources,activities,bc/study,bc/sim}/**` are bundled into the browser via SvelteKit. A top-level `import 'node:fs'` will crash the dev server with a Vite externalization error; a bare `Buffer.from(...)` or `process.env.X` will silently pass node-mode and happy-dom tests, then ReferenceError at hydration in real browsers. If you need a Node built-in inside one of these libs, lazy-load it via `process.getBuiltinModule('node:fs')` inside a function body, gated behind a `typeof process !== 'undefined'` check (canonical pattern: [libs/constants/src/source-cache.ts](libs/constants/src/source-cache.ts)). For server-only files that legitimately live inside a browser-bundled lib (CLI ingest, server-side cache, logger), tag the file with a top-of-file comment `// @browser-globals: server-only -- never imported by client .svelte` and verify the file is not in the client bundle. Biome's `noNodejsModules` enforces the static import rule; [scripts/check-browser-globals.ts](scripts/check-browser-globals.ts) enforces the bare-globals rule AND blocks runtime imports of `@ab/db/connection`, `@ab/bc-study/server`, `@ab/bc-study/build`, `postgres`, and `node:*` from any client-eligible file under `apps/*/src/**`. Type-only imports (`import type { ... }`) are allowed because they erase at compile time.
