...
| [stage-status.md](stage-status.md) | Per-corpus pipeline stage status (where each corpus is along the 1→5 pipeline). Snapshot. |
| [inventory.md](inventory.md) | Per-asset cached source listing (auto-generated by `bun run sources inventory`) |
