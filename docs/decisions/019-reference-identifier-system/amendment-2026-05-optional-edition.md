...
#### Well-known citation fields (non-sentinel)
...
The supersedes chain handles same-document edition advances automatically: when 3B is superseded by 3C, an unpinned citation to AFH chapter 3 resolves to 3C with no per-citation work and the chain quietly does its job. `redirected_from` is for the cases the chain cannot handle on its own -- cross-edition chapter renumberings ("chapter 4 in 3B is chapter 7 in 3C"), cross-book moves ("this used to live in the AFH and now lives in the AIH"), and any other override that changed the citation target. The chain is a registry property; `redirected_from` is per-citation, captured by the human who made the override decision.
...
5. Well-known citation field `redirected_from` added to the structured citation shape. Validator gatekeeper accepts the field, parses the value via `parseIdentifier` (ERROR on un-parseable URI; no registry lookup), and the migration script populates it on every proposed rewrite. The renderer surfaces it as a small annotation under the citation title. Future-proofed for list-valued redirects but ships single-value-only.
6. Citation renderer resolves URIs and renders titles from the registry. Same change fixes the "card shows 'Handbook' instead of 'Airplane Flying Handbook'" surface bug. **Depends on step 4** -- the renderer reads the new schema shape; ship 4 first or in the same PR.
7. Migration script converts free-text legacy citations to structured form. Per D2, the script does **not** auto-apply: it produces `course/knowledge/.migration-review.md` with one row per legacy citation, the proposed rewrite, the matched sentinel value, and a checkbox. The script applies a citation only when its checkbox is ticked.
8. Author walks the migration review list, ticks each box, commits. Re-run the script to apply ticked rows.
9. **Gated flag day, in its own commit.** Legacy `source:` graduates from WARNING to ERROR and the field is removed from the schema. This step ships only after step 8's review queue closes with zero residual legacy citations; the step's PR description quotes that count (`Legacy citations remaining: 0`). Single-commit so any failure (residual unmigrated citations) is one revert away.
10. AFH-3B-ingestion work package is updated based on what migration revealed. If zero edition-pinned citations remain after migration, the WP is cancelled (the YAML stub deleted, `course/references/handbooks-noningested.yaml` deleted).

**Parallelism:** steps 3 and 7 can parallelize (validator/resolvers vs. migration script -- different files, different review surfaces). Step 6 must follow step 4. Everything else is serial.
