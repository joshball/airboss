...
- **M (`sim.attempt.scenario_id` shape CHECK `~ '^sce_'`).** Triggered next time `sim.attempt` schema is touched, or superseded when `sim_scenario` materializes as a real table (FK replaces the shape check); defense-in-depth only.
