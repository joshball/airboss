...
- Work in isolated worktrees. One worktree per Wave.
- Wave 1 merges before Wave 2 starts.
...
Execute Wave 1 and Wave 2 sequentially, each in its own isolated worktree. Merge Wave 1 before starting Wave 2. For each wave: create a branch, do the work, run `bun run check`, push, open a PR with `gh pr create`, squash-merge with `--delete-branch`, then clean up the worktree and local branch. No AI attribution anywhere.

When you hit something the handoff doc does not cover (e.g. a token name is missing from the semantic set, an exact line number has drifted, a route structure differs), investigate and decide; do not stop to ask. The handoff is precise enough to execute; small drift is expected.

Report back after each Wave: PR URL, merge SHA, one-paragraph summary of what changed, and any findings (broken token references beyond PageHelp, z-index conflicts, layout issues that surfaced).
```
