...
Single entry point for "what should I work on?" in airboss. Refresh date: 2026-05-03 (ingestion-pipeline doc reorg + AMT defer + body_override mechanism + rename generic content files + WP-CFR + IDEAS follow-ups, all merged).
...
- **[stage5-citation-deeplink](../work-packages/stage5-citation-deeplink/spec.md) (2026-05-03).** Stage-5 cross-link payoff WP. PR #562 shipped the audit + corpusForSourceType plumbing; this WP makes citation chips actually deep-link to the right flightbag section for every seeded corpus. Repoints the citation picker + verifyTargetExists from `hangar.reference` (empty in dev) to `study.reference` + `study.reference_section` (where the real data lives), collapses `regulation_node` + `ac_reference` target types into one polymorphic `reference_section`, adds `airboss_ref` column on reference_section, wires `urlForReferenceSection` into resolveCitationTargets, updates CitationChips to render `<a href>` per-target with correct target attr, updates the public-card composer policy. Spec / design / tasks / test-plan all on disk; `status: unread` (awaiting user review).
- **[hangar-platform-dashboard](../work-packages/hangar-platform-dashboard/spec.md) (2026-05-03).** A `/platform` command center inside the hangar that aggregates platform state -- WP statuses, roadmaps, NOW.md state, ADR index, IDEAS funnel, coverage gaps, recent activity, drift, doc-health -- into one navigable dashboard. Pre-canned layout (per-app columns + timeline toggle stub for v1); panes are read-only navigation, not a third source of truth. Ships alongside the discipline to keep underlying data honest: structured frontmatter blocks added to NOW.md / ROADMAP.md per app / ADR frontmatter (one-time backfill); CLAUDE.md updates for platform-tracking + 500-line doc-size cap; new `/wp-drift` skill with content-hash checkpointing so agents don't redo validation work; `bun run docs:size-check` script. Depends on hangar-review-queue Phases 1-3 (substrate, loader, docs browser). Spec / design / tasks / test-plan / user-stories on disk; `status: ready-for-review`.
