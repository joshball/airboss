...
Single entry point for "what should I work on?" in airboss. Refresh date: 2026-05-01 (library-by-cert waves 1-4 + library-completeness v1-v3 + handbooks-extras pipeline).
...
- **[library-substrate (WP-SUB)](../work-packages/library-substrate/) (2026-05-01).** PR #393 authored. The foundation WP for [library-completeness](../work-packages/library-completeness/spec.md) sequence step #1: rename `handbook_section` -> `reference_section` (+ figure, errata), drop handbook-shaped CHECK constraints, add `section_schema` + `metadata` jsonb + `depth`, generalize the seeder for both section-tree and whole-doc manifests, rewrite `getReadableReferenceIds()` against `content_md`. Acceptance criterion absorbs the former WP-EX-Verify: `bun run db seed` produces rows for all 9 handbooks (3 sectioned + 6 whole-doc extras). Awaiting ratification + merge of #393. Unlocks every follow-on library-completeness WP (WP-MTN, WP-AIM, WP-CFR-V, WP-AC-V, WP-ACS-V, WP-CC, WP-NTSB-ALJ, WP-SAFO, WP-INFO, WP-AC-FULL, WP-O8900-V5, WP-SAFETY-BRIEF).
