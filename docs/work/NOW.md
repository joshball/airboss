...
- **[library-substrate (WP-SUB) plan ratified + on main (PR #393)](../work-packages/library-substrate/spec.md).** Foundation WP for [library-completeness](../work-packages/library-completeness/spec.md) sequence step #1. Plan documents the rename (`handbook_section` -> `reference_section` + figure + errata), the schema relax (drop handbook-shaped CHECK constraints, add `section_schema` + `metadata` jsonb + `depth`), the seeder generalization (two manifest shapes from day one: section-tree vs whole-doc), and the `getReadableReferenceIds()` rewrite against `content_md`. Phasing in [tasks.md](../work-packages/library-substrate/tasks.md) (Phase A schema -> G ship). Build is the next step; deferred until the in-flight sources-review fixer wave (5 open PRs) lands to avoid rebases.
...
- **[library-substrate (WP-SUB)](../work-packages/library-substrate/) (2026-05-01).** Plan ratified + on main via PR #393. Foundation WP for [library-completeness](../work-packages/library-completeness/spec.md) sequence step #1: rename `handbook_section` -> `reference_section` (+ figure, errata), drop handbook-shaped CHECK constraints, add `section_schema` + `metadata` jsonb + `depth`, generalize the seeder for both section-tree and whole-doc manifests, rewrite `getReadableReferenceIds()` against `content_md`. Acceptance criterion absorbs the former WP-EX-Verify: `bun run db seed` produces rows for all 9 handbooks (3 sectioned + 6 whole-doc extras). **Build pending; deferred until the sources-review fixer wave (5 open PRs) lands** to avoid rebases against in-flight `libs/sources/`/`libs/aviation/` changes. Phasing in [tasks.md](../work-packages/library-substrate/tasks.md) (Phase A schema -> G ship). Unlocks every follow-on library-completeness WP (WP-MTN, WP-AIM, WP-CFR-V, WP-AC-V, WP-ACS-V, WP-CC, WP-NTSB-ALJ, WP-SAFO, WP-INFO, WP-AC-FULL, WP-O8900-V5, WP-SAFETY-BRIEF).
- **[promotion-batches-persistence](../work-packages/promotion-batches-persistence/spec.md) (2026-05-02).** Cluster H closure WP authored via PR #404. Replaces the in-memory `promotion_batches` + `EDITIONS` Maps in `libs/sources/src/registry/` with Postgres-backed tables; freezes the audit-row contract from ADR 019 §2.4. Build sequenced after W5 (registry index) + W6 (atomic in-memory writes) land so the persistent design supersedes them rather than partial-undoes them.
