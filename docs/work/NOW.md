...
- **handbook-ingestion-and-reader (ADR 016 phase 0)** -- PHAK + AFH + AvWX ingested at section-granularity (1861 `handbook_section` rows total), reader UI under `/handbooks/...` with chapter/section views + read-state + heartbeat + suggestion banner, bidirectional citation surface on knowledge node detail. Storage policy [ADR 018](../decisions/018-source-artifact-storage-policy/decision.md) (Flavor D: cache + dormant LFS plumbing) and edition+errata policy [ADR 020](../decisions/020-handbook-edition-and-amendment-policy.md) shipped alongside. Two ingestion strategies committed (TOC deterministic + LLM via committed prompt); see [HANDBOOK_INGESTION_STRATEGIES.md](../platform/HANDBOOK_INGESTION_STRATEGIES.md). WP: [docs/work-packages/handbook-ingestion-and-reader/](../work-packages/handbook-ingestion-and-reader/spec.md). Open PR: tbd. AFH MOSAIC errata located but not applied (deferred to first run of the `--apply-errata` flow per ADR 020).
...
- **reference-identifier-scheme-validator (ADR 019 phase 1)** -- shipped. New `@ab/sources` lib at `libs/sources/` with the `airboss-ref:` URI parser, the 15-rule validator (§1.5), and the lesson Markdown walker (§3.4). Wired into `bun run check` via `scripts/airboss-ref.ts`; today scans 25 lessons under `course/regulations/`, finds 0 identifiers, exits clean. Phase 2 (`reference-source-registry-core`) is next -- it implements `RegistryReader` against the constants table so the validator's row 2/3/4 stop tripping. Spec: [docs/work-packages/reference-identifier-scheme-validator/](../work-packages/reference-identifier-scheme-validator/spec.md).
