...
The data layer already exists on main. The composer ships the three routes (`/program/goals`, `/program/goals/new`, `/program/goals/[id]`) and the UI that drives goal mutations through the existing `goals.ts` BC. No new schema, no new constants. The one exception is the Phase 5 course-tab actions, which write the `goalCourse` table inline in the detail `+page.server.ts` rather than through a BC wrapper.
...
| Detail + edit       | [apps/study/src/routes/(app)/program/goals/[id]/](../../../../apps/study/src/routes/(app)/program/goals/%5Bid%5D/) (13 form actions: `update`, `setStatus`, `makePrimary`, `archive`, `addSyllabus`, `removeSyllabus`, `setSyllabusWeight`, `addNode`, `removeNode`, `setNodeWeight`, `addCourse`, `removeCourse`, `setCourseWeight`) |
| BC                  | [libs/bc/study/src/goals.ts](../../../../libs/bc/study/src/goals.ts) (300+ lines) + [goals.test.ts](../../../../libs/bc/study/src/goals.test.ts). Node-picker results come from `listNodesWithFacets` in [libs/bc/study/src/knowledge.ts](../../../../libs/bc/study/src/knowledge.ts), not `goals.ts`.                                |
...
| No new BC functions for the original goal CRUD.                                | The goal/syllabus/node mutations route through existing `goals.ts` BC functions. Composer is pure UI over that. Exception: the Phase 5 course-tab actions (see next row).                                                                                                                                            |
| Courses tab from course-reader-and-editor WP Phase 5 lands here.               | Distinct from syllabi + nodes -- a goal can include curated course paths with per-link weight. Handled via `addCourse`, `removeCourse`, `setCourseWeight` form actions, which write the `goalCourse` table inline in the detail `+page.server.ts` rather than through a BC wrapper.                                  |
