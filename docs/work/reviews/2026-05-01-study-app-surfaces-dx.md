...
Re-greped main. 11 of 18 closed; 7 still-open after the second sweep. PRs:
[#585](https://github.com/joshball/airboss/pull/585) closed the verb-entity log-quality cluster;
this PR closes the remaining log-quality items (rows 27, 28, 30, 32, 34, 36, 37). The
banner findings (handbook `.catch(() => null)`, login 5xx branch, discovery promotion,
sign-out body snippet, `narrow`/`coerceEnum` dedup, RenderedLesson memoisation) remain
on the tail and are tracked in `docs/work-packages/review-tail-2026-05/`.
...
| MAJOR    | Error log messages are non-grep-friendly verbs | CLOSED | PR #585 -- mechanical sweep across memory/reps/plans/sessions catches; user-visible noun phrase aligned in both `log.error` and `fail()` messages |
...
| MAJOR    | discovery spawn errors logged at `warn`, not `error` | CLOSED | `apps/study/src/lib/server/discovery.ts:60,66` -- both branches now `log.error('spawn discovery child failed', ...)` |
...
| MINOR    | Banned-user log uses `warn` per-request (no rate limit) | CLOSED | `hooks.server.ts` -- downgraded to `log.info('banned user blocked', ...)`; the 403 is expected behaviour, no longer pages on volume. Counter wiring deferred until the rate climbs |
| MINOR    | applySecurityHeaders swallows misconfiguration | CLOSED | `hooks.server.ts:applySecurityHeaders` now early-returns on `TypeError` (frozen-headers branch) and logs `log.error('set security headers failed', ...)` in prod for any other throw; dev keeps the noisier `warn` |
...
| MINOR    | Card-page client-side fetch nested try/catch loses context | CLOSED | `_panels/CardCitationsPanel.svelte:64-73` -- nested try/catch collapsed; `Error(message, { cause: { status, url } })` carries HTTP context for upstream debuggers; user-safe string preserved |
...
| MINOR    | handbook redirect uses 308 with no logging | CLOSED | `handbooks/[...rest]/+server.ts` -- emits `log.info('legacy handbooks redirect', { metadata: { from, to } })` per request; retirement signal for the legacy shim per `feedback_no_legacy_in_airboss` |
...
| NIT      | cards/[id] server load doesn't tag a logger | CLOSED | `cards/[id]/+page.server.ts` -- `createLogger('study:public-card')` + try/catch that re-throws SvelteKit `HttpError` and tags real failures (DB, citation resolver) with `cardId` metadata |
...
| NIT      | handbook-asset 404 generic for three distinct cases | CLOSED | `handbook-asset/[...path]/+server.ts` -- traversal branch logs `log.warn('handbook-asset path-escape attempt', { metadata: { requested } })`; missing-file branches stay quiet |
| NIT      | appearance/theme POST 400 paths don't log | CLOSED | `appearance/+server.ts` + `libs/themes/picker/server.ts:createThemeEndpoint` -- both 400 branches (bad JSON + invalid value) now emit `log.info` with `requestId/userId/metadata` so a broken client surfaces in telemetry without paging |
