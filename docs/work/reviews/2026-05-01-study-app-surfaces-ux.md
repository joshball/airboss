...
Re-greped main. 5 of 18 closed; 13 still-open. Most remaining items need product-shape decisions (card-state indicator, library spine empty-state UX) or are minor polish.

| Severity | Finding                                                                                  | Verdict    | Evidence                                                                                                                                                                                                                                                                                             |
| -------- | ---------------------------------------------------------------------------------------- | ---------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| MAJOR    | /library/topic/[topic] 404s on unknown topic instead of soft empty                       | STILL OPEN | `library/topic/[topic]/+page.server.ts:69` still throws 404 (slug echo dropped per dx review). Next: needs UX decision -- soft empty + back-link, or 404 + recovery affordance in `+error.svelte`                                                                                                    |
| MAJOR    | Library landing has no card-state indicator (spec smell #5 / Open Q #1)                  | STILL OPEN | spec follow-on. Next: add per-spine readable counts via `getReadableReferenceIds()`, surface as sub-line. Trigger: library-completeness Wave-2                                                                                                                                                       |
| MAJOR    | /library/regulations empty buckets render as 0.55-opacity links to dead lists            | STILL OPEN | `library/regulations/+page.svelte:64` still `opacity: 0.55` with no `aria-disabled`. Next: filter `count > 0` mirroring landing-page pattern, OR convert to disabled-treatment with explicit "no references yet" copy                                                                                |
| MAJOR    | /library/aircraft/[slug] gives no in-app content path even when isReadable would be true | STILL OPEN | hardcoded `isReadable={false}`. Same shape on `library/regulations/[kind]/+page.svelte:67` (umbrella refs). Next: drive `isReadable` from loader                                                                                                                                                     |
| MAJOR    | Form-error surfacing inconsistent across pages                                           | STILL OPEN | `memory/new/+page.svelte:84`, `plans/new/+page.svelte:80`, `sessions/[id]/+page.svelte:118` all still use `<div class="error">`. `plans/[id]/+page.svelte:111` is on `Banner tone="danger"`. Next: standardize on `Banner tone="danger"` for form-level errors; `<span class="err">` for field-level |
| MAJOR    | Memory/review mid-flow loading state invisible during route transitions                  | CLOSED     | `memory/review/[sessionId]/+page.svelte:262-280` -- rating shortcut path is gated `phase === REVIEW_PHASES.ANSWER` so SUBMITTING input is dropped. The buttons themselves are `disabled={phase !== ANSWER}`                                                                                          |
| MAJOR    | /sessions/[id] "Skip permanently" terse confirm copy                                     | STILL OPEN | `sessions/[id]/+page.svelte:294-302` -- still `confirmLabel="Skip permanently (undo from plan)"` + hint paragraph. Next: enrich with `confirmBody` referencing the plan-detail Reactivate path                                                                                                       |
| MINOR    | Handbook reader "newer edition" banner role="alert" for passive notice                   | CLOSED     | `library/handbook/[slug]/+page.svelte:34` -- `role="status"`                                                                                                                                                                                                                                         |
| MINOR    | Handbook "newer edition" link points to itself                                           | STILL OPEN | `library/handbook/[slug]/+page.svelte:37` still uses `ROUTES.LIBRARY_HANDBOOK(slug)`. Next: loader exposes `supersededByDocumentSlug`                                                                                                                                                                |
| MINOR    | /library aircraft section disappears when count = 0                                      | STILL OPEN | `library/+page.svelte:135` still hides when empty. Needs UX decision                                                                                                                                                                                                                                 |
| MINOR    | /library/regulations/[kind] umbrella refs hardcoded `isReadable={false}`                 | STILL OPEN | tied to MAJOR aircraft finding -- same root cause                                                                                                                                                                                                                                                    |
| MINOR    | Memory/new "Save and add another" intent unclear                                         | STILL OPEN | needs differentiated redirect for the two intents                                                                                                                                                                                                                                                    |
| MINOR    | Pending session preset submit "Starting..." UX                                           | STILL OPEN | low priority                                                                                                                                                                                                                                                                                         |
| MINOR    | Plans/new range slider lacks aria-valuetext                                              | CLOSED     | `plans/new/+page.svelte:161` -- `aria-valuetext="{sessionLength} items per session"`                                                                                                                                                                                                                 |
| MINOR    | Dashboard footer timestamp captured at module load                                       | CLOSED     | `dashboard/+page.svelte:19-30` -- now ticks on a `$state` clock, `$derived` stamp                                                                                                                                                                                                                    |
| MINOR    | Sessions/[id] "Skip today" no confirm vs "Skip topic"/"Skip permanently"                 | STILL OPEN | UX visual hierarchy decision needed                                                                                                                                                                                                                                                                  |
| NIT      | Library landing "Show all topics" copy missing aria-expanded                             | CLOSED     | `library/+page.svelte:113-114` -- `aria-expanded={showAllTopics}` and `aria-controls="library-topics-list"`                                                                                                                                                                                          |
| NIT      | Memory/[id] padding inconsistent with /memory/review card                                | STILL OPEN | minor token consistency. Trigger: design-system pass                                                                                                                                                                                                                                                 |
| NIT      | /credentials "no primary goal" banner has no dismiss                                     | STILL OPEN | low priority. Trigger: power-user feedback                                                                                                                                                                                                                                                           |
