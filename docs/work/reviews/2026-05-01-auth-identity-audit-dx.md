...
- **Pretty-mode logger metadata layout**: dev-only papercut. Trigger: when interleaved-request log noise becomes a real on-call gripe. Inline `metadata` into the formatted line then.
- **`auditWrite` actor-id provenance**: design question (discriminated union vs. required `metadata.systemSource`). Trigger: first system-owned audit write that needs to be distinguishable from a forgotten `requireAuth` in the audit explorer.
