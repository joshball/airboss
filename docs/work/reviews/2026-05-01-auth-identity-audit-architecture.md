...
- **`@ab/audit/schema` deep-import**: low-risk but not closed. Trigger to land: any future change that moves `bauthUser` inside `@ab/auth` (would force the audit lib to follow). Drop-in fix is `from '@ab/auth'` once we confirm the barrel path is stable.
- **`auditColumns` placement + `auditSchema` export**: dormant -- `auditColumns` has no live call sites yet, and `auditSchema` has no external consumer. Trigger: when the first BC adopts `auditColumns()` (then move + rename in one pass) or when an external import for `auditSchema` lands (then drop the export immediately). Both are tiny mechanical fixes once a real consumer exists.
