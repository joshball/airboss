...
| -------- | ------- | ------- | -------- |
| CRITICAL | references.test.ts historical-lens tests don't assert default `kind === 'current'` | CLOSED | `apps/study/src/lib/server/references.test.ts:92` (positive default-pin in the existing serialization test) + `apps/study/src/lib/server/references.test.ts:98-122` (dedicated regression-bait test "defaults annotation kind to `none` ... when no acks or historicalLens are passed", with `toMatchObject` on `{kind: 'none', text: ''}` and `note` undefined check). The codebase's `AnnotationKind` enum is `'none' \| 'covered' \| 'chain-advanced' \| 'historical' \| 'cross-corpus'` -- there is no `'current'` value; `'none'` is the visual-current default per `libs/sources/src/render/modes/web.ts` (no annotation span emitted). Regression-bait verified: temporarily flipping the default branch in `libs/sources/src/render/annotations.ts:99` to `'historical'` fails exactly these two assertions, while the four other tests in the file (round-trip / plain-text / acks / lens) still pass. PR #468 lineage; review-tail Tier 1 §B. |
