...
Re-greped main. 5 of 8 closed (was 2). Chunk-5's close-out audit (PR #568) shipped the convergent effect-mirror fix for 5 +layout.svelte files including this one, closing the effect-mirror MAJOR + the linked `state_referenced_locally` MINOR. The pervasive route-level CSS proliferation MAJOR is now closed at the cluster scope (Card / Toast / ScoreMeta / BadgeStatus extracted to `libs/ui/` with 6 study-app sites rewired). Remaining follow-ups: URL side-effects MAJOR + module-scoped timer MINOR (page-level files), IdentityMenu extraction (separate work-package).

| Severity | Finding | Verdict | Evidence |
| -------- | ------- | ------- | -------- |
| MAJOR    | Pervasive heavy visual CSS in route files (65 files affected) | CLOSED 2026-05-04 (chunk-1 svelte cluster) | Card already on main; Toast / ScoreMeta / BadgeStatus extracted into `libs/ui/src/components/` with token-driven styles, harness tests (23 passing), and type re-exports on the `@ab/ui` barrel. Six consumer sites rewired: memory/review/[sessionId] (share-toast + undo-toast); memory/[id]/_panels/CardDetailPanel (edit + share toasts); calibration (score-meta dl); plans (active + archived status pills); plans/[id] (subtitle status pill). IdentityMenu and a separate token-migration finishing pass remain as separate work-packages per project rule. |
