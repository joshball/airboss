...
Re-greped main. 4 of 8 closed (was 2). Chunk-5's close-out audit (PR #568) shipped the convergent effect-mirror fix for 5 +layout.svelte files including this one, which closes the second MAJOR plus the linked MINOR about `state_referenced_locally` recurrence in the same layout. Route-level CSS proliferation MAJOR remains the dominant follow-up; URL side-effects MAJOR + module-scoped timer MINOR still live in page-level files.
