...
- **Goal references a course AND a syllabus that both link the same node**: `getGoalNodeUnion` deduplicates by `knowledge_node_id` and sums weights. Same semantic the goal_syllabus + goal_node aggregation already uses.
