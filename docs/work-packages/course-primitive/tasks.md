...
- [ ] Add `COURSE_KINDS`, `COURSE_KIND_VALUES`, `CourseKind`, `COURSE_KIND_LABELS` to `libs/constants/src/credentials.ts` (alongside `SYLLABUS_KINDS`).
- [ ] Add `COURSE_STATUSES`, `COURSE_STATUS_VALUES`, `CourseStatus` to `libs/constants/src/credentials.ts`.
- [ ] Add `COURSE_STEP_LEVELS`, `COURSE_STEP_LEVEL_VALUES`, `CourseStepLevel` to `libs/constants/src/credentials.ts`.
- [ ] Add `KNOWLEDGE_NODE_KINDS`, `KNOWLEDGE_NODE_KIND_VALUES`, `KnowledgeNodeKind`, `KNOWLEDGE_NODE_KIND_LABELS` to `libs/constants/src/study.ts` (next to `NODE_LIFECYCLES`).
- [ ] Re-export the new constants from `libs/constants/src/index.ts`.
- [ ] Add `ROUTES.COURSES` (static), `ROUTES.COURSE(slug)` (function), `ROUTES.COURSE_STEP(slug, stepCode)` (function) to `libs/constants/src/routes.ts`. Mirror the syllabus / credential function shape.
- [ ] Run `bun run check` -- 0 errors. Commit.

### Phase 2: Schema migration

- [ ] Create `libs/bc/study/src/schema.ts` additions for `course`, `courseStep`, `goalCourse` tables. Place after `goalNode` (line 2313) so the cert-syllabus block stays contiguous.
- [ ] Add `kind` column to `knowledgeNode` table (after `lifecycle`, before `referencesV2Migrated`). Default `'concept'`.
- [ ] Add CHECK constraints per spec:
...
- [ ] Add indexes per spec: `course_slug_unique`, `course_kind_status_idx`, `course_step_course_code_unique`, `course_step_tree_idx`, `course_step_node_idx`, `goal_course_by_course_idx`.
- [ ] Add `CourseRow`, `CourseStepRow`, `GoalCourseRow` type exports at the bottom of `schema.ts`.
- [ ] Generate the migration: `bun run db generate`. Verify the SQL diff in `drizzle/00XX_course_primitive.sql`.
- [ ] Apply: `bun run db migrate`. Verify tables exist + `knowledge_node.kind` is populated to `'concept'` on every existing row.
- [ ] Run `bun run check` -- 0 errors. Commit.

### Phase 3: Course BC

- [ ] Create `libs/bc/study/src/courses.ts` with public functions:
...
- [ ] Extend `getGoalNodeUnion(goalId, db)` in `libs/bc/study/src/goals.ts` to also walk `goal_course -> course_step (level='step') -> knowledge_node_id` and merge into the returned union with `goal_course.weight`. Dedup by knowledge_node_id; sum weights when a node is reached via multiple sources.
- [ ] Add unit tests in `libs/bc/study/src/__tests__/courses.test.ts`. Coverage:
  ...
  - `getGoalNodeUnion` with course + syllabus + ad-hoc node (dedup + weight sum)
- [ ] Run `bun run check` -- 0 errors, all tests pass. Commit.

### Phase 4: Lens framework extensions + courseLens

- [ ] Extend `LensLeaf` in `libs/bc/study/src/lenses.ts` with optional `sources?: LensLeafSources` field.
- [ ] Extend `LensResult` with optional `certGaps?: CertGap[]` field.
- [ ] Extend `LensTreeNode['level']` union to include `'course'` and `'section'`.
- [ ] Verify existing `acsLens` and `domainLens` still typecheck and pass tests (the new fields are optional).
- [ ] Implement `courseLens` in a new file `libs/bc/study/src/lenses-course.ts` (follows the existing per-lens file pattern that the cert-syllabus WP introduced).
- [ ] Re-export `courseLens` from `libs/bc/study/src/lenses.ts`.
- [ ] Add unit tests in `libs/bc/study/src/__tests__/lenses-course.test.ts`. Coverage:
...
- [ ] Implement `courseWithCertOverlayLens` in `libs/bc/study/src/lenses-course.ts`.
- [ ] Implement helper `getCourseGaps(goalId, courseId, syllabusId, db)` that returns the `CertGap[]` array independent of the lens (useful for non-tree consumers).
- [ ] Re-export both from `libs/bc/study/src/lenses.ts`.
- [ ] Add unit tests:
...
- [ ] Add YAML loader at `scripts/db/seed-courses.ts`. Walk `course/courses/<slug>/` directories. Mirror `scripts/db/seed-syllabi.ts` shape (or wherever the syllabus seed lives -- check the repo for the canonical file path).
- [ ] Add Zod schemas for course manifest + section + step in `libs/bc/study/src/course-yaml-schemas.ts`. Strict; reject unknown keys.
- [ ] Implement validator: every spec rejection in the "Seed validator rejections" table fires with the exact message string for log-grepping.
- [ ] Wire up `bun run db seed courses` command in `scripts/db.ts` (or wherever the db CLI dispatcher lives).
- [ ] Add a smoke test fixture: `course/courses/_fixtures/seed-smoke/` with one section + two steps + one transition step. Used by Phase 7 acceptance.
- [ ] Run `bun run check` -- 0 errors. Commit.

### Phase 7: Diagnostic + smoke acceptance

- [ ] Add `bun run db diagnose:school-personal-syllabi` command. Reads `study.syllabus` and prints any rows with `kind IN ('school','personal')`. Exit 0 with row count.
- [ ] Run the diagnostic on the dev DB. Confirm zero rows. Commit the script.
- [ ] Run `bun run db seed courses` against the smoke fixture. Verify:
...
- [ ] Re-run the seed; verify zero writes (idempotency).
- [ ] Modify one fixture step's `body_md`; re-run seed; verify exactly one update on that step (content_hash changed).
- [ ] Commit.

### Phase 8: Documentation + WP closeout

- [ ] Update `docs/work/NOW.md` with this WP's shipped status.
- [ ] Add an entry to `docs/log/` per the project's per-PR log convention.
- [ ] Update `docs/decisions/016-cert-syllabus-goal-model/decision.md` Migration Plan table with a new row for course-primitive (status: shipped, link to PR).
- [ ] Run `bun run check all` -- 0 errors, 0 warnings.
- [ ] Open the PR. Title: `feat(course-primitive): course as peer primitive to syllabus`.
