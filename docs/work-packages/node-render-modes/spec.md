...
- **No flightbag section render modes** in v1 (foreshadowed for v1.1).
- **No new BC functions for evidence**; this WP is purely a rendering / authoring change.
- **No new card / scenario authoring tooling** -- existing surfaces handle that.
- **No multi-author collaboration on node bodies.**
- **No version history of section content.** Git is the version history.
- **No A/B test of mode defaults.** Learn is the default; we don't experiment.
- **No analytics on which mode users pick.** Could add later; not v1.
- **No per-user custom mode** ("memorize-but-with-hook-collapsed-and-prompts-first"). Three fixed modes.
