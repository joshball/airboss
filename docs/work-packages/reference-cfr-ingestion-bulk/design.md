...
**Chosen:** One script: `bun run cfr-ingest`. Flags: `--edition=<YYYY-MM-DD>` (override default of "today"), `--fixture=<path>` (read XML from local file instead of network), `--out=<path>` (override default derivative root), `--help`. The script orchestrates fetch -> walk -> normalize -> write -> populate -> promote in one pass.
