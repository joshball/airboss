...
| Wind-vs-isobar consistency        | Sanity-floor check, not a balance proof. METAR winds derive from the containing air-mass literal, not from a geostrophic computation, so post-frontal cP / lake-effect / thermally driven scenarios legitimately diverge from the synoptic gradient. The check fires only on textbook-implausible cases: tolerance 170 deg for non-frontal stations, 180 deg (effectively unrestricted) within 500 km of a front. Catches catastrophic authoring errors without flagging legitimate air-mass-advection winds. See `libs/wx-engine/src/validate/consistency.ts` for inline rationale |
| TAF FM time vs front motion       | Each `FM` group's hour matches the projected front-arrival time at the station within +/- 5 hours. The engine's TAF FM detector rounds to the hour and the projection itself carries motion-vector approximation error; 5 h catches wholly-misplaced FMs (a 12-h-off bug) without flagging legitimate hour-rounding drift                                                                                                                                                                                                              |
