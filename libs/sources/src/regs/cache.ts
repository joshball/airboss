...
import { resolveCacheRoot as canonicalResolveCacheRoot } from '../cache.ts';
...
/**
 * Resolve the cache root. Thin re-export of the canonical helper in
 * `libs/sources/src/cache.ts`; kept here for callers that import from
 * `@ab/sources/regs` and for the existing `cache.test.ts` exercising
 * `resolveCacheRoot` end-to-end. Honors `AIRBOSS_HANDBOOK_CACHE`; defaults to
 * `~/Documents/airboss-handbook-cache/`. Creates the directory on demand.
 */
export function resolveCacheRoot(): string {
	return canonicalResolveCacheRoot();
}
