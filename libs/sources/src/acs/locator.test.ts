...
describe('parseAcsLocator', () => {
	it('parses a whole-publication locator', () => {
		const result = parseAcsLocator('ppl-airplane-acs-6c');
		expect(result.kind).toBe('ok');
		if (result.kind !== 'ok') return;
		expect(result.acs).toEqual({ slug: 'ppl-airplane-acs-6c' });
	});

	it('parses an area locator', () => {
		const result = parseAcsLocator('ppl-airplane-acs-6c/area-05');
		expect(result.kind).toBe('ok');
		if (result.kind !== 'ok') return;
		expect(result.acs).toEqual({ slug: 'ppl-airplane-acs-6c', area: '05' });
	});

	it('parses a task locator', () => {
		const result = parseAcsLocator('ppl-airplane-acs-6c/area-05/task-a');
		expect(result.kind).toBe('ok');
		if (result.kind !== 'ok') return;
		expect(result.acs).toEqual({ slug: 'ppl-airplane-acs-6c', area: '05', task: 'a' });
	});

	it('parses a knowledge element locator', () => {
		const result = parseAcsLocator('ppl-airplane-acs-6c/area-05/task-a/elem-k01');
		expect(result.kind).toBe('ok');
		if (result.kind !== 'ok') return;
	...
	it('parses a risk-management element locator', () => {
		const result = parseAcsLocator('cfi-airplane-acs-25/area-01/task-a/elem-r02');
		expect(result.kind).toBe('ok');
		if (result.kind !== 'ok') return;
	...
	it('parses a skill element locator', () => {
		const result = parseAcsLocator('ppl-airplane-acs-6c/area-05/task-a/elem-s03');
		expect(result.kind).toBe('ok');
		if (result.kind !== 'ok') return;
	...
	it('parses two-digit element ordinals up to 99', () => {
		const result = parseAcsLocator('cfi-airplane-acs-25/area-02/task-b/elem-k12');
		expect(result.kind).toBe('ok');
		if (result.kind !== 'ok') return;
	...
	it('rejects single-digit (non-padded) area ordinal', () => {
		const result = parseAcsLocator('ppl-airplane-acs-6c/area-5');
		expect(result.kind).toBe('error');
		if (result.kind !== 'error') return;
	...
	it('rejects roman-numeral area ordinal (the pre-Q7 PR2 format)', () => {
		const result = parseAcsLocator('ppl-airplane-acs-6c/area-v');
		expect(result.kind).toBe('error');
	});

	it('rejects malformed task segment (multi-letter)', () => {
		const result = parseAcsLocator('ppl-airplane-acs-6c/area-05/task-aa');
		expect(result.kind).toBe('error');
		if (result.kind !== 'error') return;
	...
	it('rejects element segment using the pre-Q7 `element-` prefix', () => {
		const result = parseAcsLocator('ppl-airplane-acs-6c/area-05/task-a/element-k1');
		expect(result.kind).toBe('error');
	});

	it('rejects element segment without 2-digit ordinal', () => {
		const result = parseAcsLocator('ppl-airplane-acs-6c/area-05/task-a/elem-k1');
		expect(result.kind).toBe('error');
	});

	it('rejects element segment with unknown triad letter', () => {
		const result = parseAcsLocator('ppl-airplane-acs-6c/area-05/task-a/elem-x01');
		expect(result.kind).toBe('error');
	});

	it('rejects element segment with no ordinal', () => {
		const result = parseAcsLocator('ppl-airplane-acs-6c/area-05/task-a/elem-k');
		expect(result.kind).toBe('error');
	});

	it('rejects extra segments after element', () => {
		const result = parseAcsLocator('ppl-airplane-acs-6c/area-05/task-a/elem-k01/extra');
		expect(result.kind).toBe('error');
		if (result.kind !== 'error') return;
...
describe('formatAcsLocator', () => {
	it('round-trips through parse', () => {
		const cases: ParsedAcsLocator[] = [
			{ slug: 'ppl-airplane-acs-6c' },
			{ slug: 'ppl-airplane-acs-6c', area: '05' },
			{ slug: 'ppl-airplane-acs-6c', area: '05', task: 'a' },
			{
				slug: 'ppl-airplane-acs-6c',
				area: '05',
				task: 'a',
	...
	it('truncates output when intermediate fields are missing', () => {
		...
		expect(formatted).toBe('ppl-airplane-acs-6c/area-05');
	});
});
