...
function identityOf(s: SessionItem): IdentityKey {
	...
	if (s.kind === SESSION_ITEM_KINDS.NODE_START) return { kind: SESSION_ITEM_KINDS.NODE_START, id: s.nodeId };
	// teaching-exercise: not generated by the engine today (evidence-kind-data-layer
	// WP ships the substrate; engine pickup is a separate WP). Identity falls
	// back to the teachingExerciseId so seed-inserted rows still de-dupe correctly.
	return { kind: SESSION_ITEM_KINDS.TEACHING_EXERCISE, id: s.teachingExerciseId };
}
