...
const COMMAND_HELP: Record<string, CommandHelp> = {
	...
	seed: {
		...
		how: 'The orchestrator (scripts/db/seed-all.ts) shells out to `scripts/db/seed-dev-users.ts` and `scripts/build-knowledge-index.ts`, imports `seedCardsForUser` from `scripts/db/seed-cards.ts` once per DEV_ACCOUNTS entry, and imports `seedCourses` from `scripts/db/seed-courses.ts` to walk every `course/courses/<slug>/` directory. The `courses` sub-target also shorts to `scripts/db/seed-courses.ts` directly so a single-slug iteration loop can skip the other phases.',
		links: [
